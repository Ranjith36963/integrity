...
lib/                 Domain logic: types, data, scoring, utilities
  ...
  currentDayBlocks.ts Pure helper: filters today's blocks by the deletions map (M5)
  currentDayView.ts  Pure "today" projection: visibleDayBlocks() = currentDayBlocks ∩ appliesOn(recurrence, currentDate); currentDayState() for scoring — the Day view/ring/score show only today's routine (resolves ADR-047)
