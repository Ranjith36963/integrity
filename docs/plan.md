...
One-line summary (per ADR-043, supersedes ADR-035 + M4c timer infrastructure): collapse `Brick` to `kind: "tick" | "units"`, rename `count → done`, drop `runningTimerBrickId` and four timer actions plus `LOG_GOAL_BRICK` from the `Action` union, add `SET_UNITS_DONE`, delete `lib/timer.ts` / `<TimerSheet>` / `useTimer` / placeholder chime / `<BrickStepper>` / dead-code `overlapsExistingBlock`, and add one new component `<UnitsEntrySheet>` that opens on a single tap of any units `<BrickChip>`. No persistence migration needed; M4e schema additions (`hasDuration`, `start`, `end`, `recurrence`) on `BrickBase` are preserved untouched.
...
DELETE  public/sounds/chime.mp3                         (SG-m4f-05: no celebratory sound in M4f; M4a's playChime usage also removed — see § Audio cleanup)
DELETE  lib/audio.ts                                    (sole consumer of chime.mp3; replaced by inert stub at call sites)
DELETE  lib/audio.test.ts                               (paired with lib/audio.ts)
...
| `chime` (broader sweep)                                           | 8     | DELETE `lib/audio.ts` + `lib/audio.test.ts` + `public/sounds/chime.mp3`; remove `playChime()` calls in `Fireworks.tsx` / `TimelineBlock.tsx` / `BuildingClient.tsx` and the comments referencing chime in `lib/celebrations.ts`. See § Audio cleanup below — this is a clarifying scope expansion beyond the spec's "delete `public/chime.mp3`" wording (no file at that exact path exists; the actual placeholder is `public/sounds/chime.mp3`). |
...
- `lib/audio.ts` + `lib/audio.test.ts` + `public/sounds/chime.mp3` — DELETE (see § Audio cleanup for the scope expansion rationale; SG-m4f-05).
...
Total surface: **18 production files edited, 8 files outright deleted, 13 test files migrated, 2 test files deleted outright, 1 new component file, 1 new test file** (the latter written by BUILDER in TDD). Plus `docs/decisions.md` (ADR-042 prose harmonization) and `docs/status.md` (SHIPPER updates).
...
**Day-100% cross-up.** The `useCrossUpEffect` for day-100% (line 118 currently fires haptic + chime + fireworks) loses its chime call per § Audio cleanup; the haptic + fireworks remain. The cross-up math is unchanged.
...
lib/audio.ts                                    (sole consumer of chime.mp3; SG-m4f-05 → no celebratory sound)
lib/audio.test.ts                               (paired)
public/sounds/chime.mp3                         (the actual file; the spec's "public/chime.mp3" path is incorrect — see § Audio cleanup)
...
The spec instructs "delete `public/chime.mp3`" (AC #14). The file at that exact path does NOT exist — `ls /home/user/integrity/public/` returns `icon.svg` and `sounds/`. The actual placeholder lives at **`public/sounds/chime.mp3`** (431 bytes, per `docs/status.md:52`).

That placeholder is consumed by **`lib/audio.ts:playChime()`**, which is invoked from three sites:

1. `app/(building)/BuildingClient.tsx:118` — M4a day-100% cross-up effect.
2. `components/TimelineBlock.tsx:64` — M4a block-100% cross-up effect.
3. M4c's timer (via the M4c plan but actually wired through `lib/celebrations.ts` cross-up registry, not directly).

The M4c timer chime path is what ADR-043 actually intends to delete. The M4a block/day chime path is an independent feature. **However, SG-m4f-05 ("Replace M4c's chime with anything for units bricks? Recommendation: no") combined with `docs/status.md:52`'s open loop ("`chime.mp3` is a 431-byte placeholder. Replace with a real royalty-free chime before any M4a/M4b/M4c/M4d preview is user-facing") suggests the chime is broadly unwanted in M4f — the placeholder was never replaced and the M4c-removal pulls the rug.**

Plan-locked: **delete `lib/audio.ts`, `lib/audio.test.ts`, `public/sounds/chime.mp3` outright; strip the `playChime()` calls from `BuildingClient.tsx`, `TimelineBlock.tsx`, and any path in `lib/celebrations.ts`.** Haptic + fireworks for the M4a/day-100% and M4a/block-100% cross-ups remain (visual + tactile celebrations); the audio celebration is gone in M4f. A future M7 milestone can reintroduce audio with a real royalty-free asset and explicit user-celebratory intent.

**This is a clarifying scope expansion** beyond the spec's literal wording but consistent with ADR-043's spirit ("the app does not count for the user") and resolves the long-standing `chime.mp3` open loop. **Flag for VERIFIER** in § Open questions — this is the only scope decision in M4f that isn't already locked verbatim by the spec or an ADR.
...
5. **Phase E — file deletions + audio cleanup.** Once nothing references the deleted files, delete them in a single commit (`feat(m4f): rip M4c timer infrastructure`). Run `npx tsc --noEmit` after this commit; expected: zero errors.
...
| ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `public/sounds/chime.mp3` placeholder (line 52)                          | RESOLVED — file deleted; `lib/audio.ts` deleted; all `playChime()` call sites stripped.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **SG-m4f-05 — Replace chime for units bricks?** RESOLVED: no audio in M4f. See § Audio cleanup for the full scope decision — the M4a/M4b/M4d-shared `public/sounds/chime.mp3` placeholder is deleted alongside the M4c timer chime, because it was never replaced and the M4f rip is the natural opportunity. A future M7 polish milestone can reintroduce audio with a real asset.
...
- Stale `chime.mp3` import → typecheck failure on `lib/audio.ts` deletion surfaces any forgotten import.
...
- **R5 — Audio cleanup scope expansion (R, but worth flagging).** Deleting `lib/audio.ts` + `public/sounds/chime.mp3` + the three `playChime()` call sites is broader than the spec's literal "delete public/chime.mp3" wording (see § Audio cleanup). **Mitigation:** rationale is fully documented; VERIFIER may accept or scope down to just deleting the M4c timer's chime call (leaving M4a's block/day chime calls intact). If VERIFIER scopes down, BUILDER skips Phase G's audio-cleanup commits and `lib/audio.ts` survives. The plan defaults to full deletion because the open loop in `status.md:52` argues the placeholder should not survive a refactor anyway.
...
Only two items the spec does not lock verbatim:

1. **Audio cleanup scope (R5).** § Audio cleanup expands the spec's literal "delete `public/chime.mp3`" (a path that does not exist) to "delete `public/sounds/chime.mp3` + `lib/audio.ts` + `lib/audio.test.ts` + strip all `playChime()` call sites." Rationale documented; consistent with `status.md:52` open loop and SG-m4f-05. VERIFIER may PASS as-is or SCOPE DOWN to "only delete the M4c-timer chime invocation," leaving `lib/audio.ts` + the M4a block/day chime calls intact. Either path satisfies AC #14 (the spec's named file doesn't exist; both interpretations land the timer chime in the trash). Plan-locked preference: full deletion (because the placeholder was never replaced; clean now or carry indefinitely).
