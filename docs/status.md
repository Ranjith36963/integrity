...
- **Branch:** `claude/verify-m0-deployment-s4XRy` at `[ship commit — see below]` · `main` at `c3ef9f1` (feature branch not yet merged)
- **Last commit:** ship commit for M3 — `docs(ship-m3): update README, CHANGELOG, status.md for M3` (stacked on `5eac7ca`)
