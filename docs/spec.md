...
After M4e shipped, the user clarified the brick model: **there are only two brick kinds, not three.** A brick is either a **tick** (boolean — "did you do it, yes / no") or a **units** brick (target + unit + done — "you set a target of N <unit>, you type how much you did today"). The "time" kind shipped in M4c — with its live timer, START/STOP actions, single-running invariant, visibilitychange recovery, and chime — is **wrong**. The user does not want the app counting minutes for them. They want to type "20" into a brick that has a target of "30 minutes running." A time-of-day window (when in the day this runs) is already covered by M4e's `hasDuration` axis. A performance target in minutes is just a units brick where the user's unit happens to be the word `"minutes"`. Rep-based goals, page counts, glasses of water, deep breaths — all the same shape, distinguished only by the unit string the user types.
...
- `lib/timer.ts`, `<TimerSheet>`, `useTimer` hook, and the `public/chime.mp3` placeholder are **deleted**.
...
- The full M4c surface — `lib/timer.ts`, `<TimerSheet>`, `useTimer`, 4 timer actions, `runningTimerBrickId`, `public/chime.mp3` — all **inputs to be deleted** by this milestone.
...
| `public/chime.mp3` (DELETED)                                             | Zero-byte placeholder from M4c. No consumer after timer removal.                                                                                                                                                                                                                                                                                                                                                                                 | Open loop in `docs/status.md` resolved.                                                                                       |
...
- **chime.mp3 import remains in any source file** → typecheck/lint error; deleted file produces a build failure that surfaces the stale import.
...
14. `public/chime.mp3` is deleted.
15. Any `useTimer` hook file (`hooks/useTimer.ts` or equivalent) is deleted.
16. No source file in `lib/`, `components/`, or `app/` imports from any deleted path. `grep -rn "lib/timer\|TimerSheet\|useTimer\|chime\.mp3" lib components app` returns zero hits post-migration.
...
42. Playwright (deferred-to-preview, M4a/b/c/d/e pattern):
    ...
    - E-m4f-05 — No live timer is observable anywhere in the app — no auto-incrementing minutesDone, no chime, no `<TimerSheet>` long-press behavior.
43. A11y:
