...
**Context.** ADR-034 (2026-05-05) hard-locked "blocks always timed, bricks never timed." That clean split worked through M2/M3/M4a/M4b/M4c/M4d. After M4c shipped and Gate #2 on the M4 trilogy completed, the user's mental model shifted: **duration is not a property of "what kind of task" but a universal time-window axis that applies to any task.** A block always has duration (its window on the timeline); a brick _might_ have duration (a window of its own) and might not. Whether a brick has duration is independent of its score type (tick / goal / time). Tick / goal / time are _performance_ axes — how the user measures progress inside the window. Duration is _scheduling_ — when the activity occupies the day.
...
The toggle is orthogonal to score type — tick, goal, and time bricks all expose the same toggle with the same semantics. For time bricks specifically, `durationMin` (introduced in M4c as the timer target) is preserved unchanged and means "performance target inside the allocated window." The new `start`/`end` is the allocated window itself. Both fields coexist on a time brick; they describe different things.
...
- **`durationMin` retains its M4c meaning.** It is NOT re-derived from `end - start`. Time bricks now have two distinct numbers: allocated window (`end - start`) and performance target (`durationMin`). The timer measures `minutesDone` against `durationMin`, untouched.
...
- **Score-type logic is untouched.** M4a tick / M4b goal / M4c time reducer arms unchanged. Cascade, scoring, hero ring, BlueprintBar — all untouched.
