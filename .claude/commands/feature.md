...
Interpret `$ARGUMENTS` as a milestone slug (e.g., `m10`, `m7f`, `m5b`). The slug determines the milestone folder under `docs/milestones/m{slug}/`.

1. Read `docs/milestones/m{slug}/spec.md`, `docs/milestones/m{slug}/decisions.md` (if it exists), `docs/milestones/_general/decisions.md`, `docs/status.md`, `phase1plan.md`. Confirm what `$ARGUMENTS` maps to. If the slug folder doesn't exist or has no `spec.md`, **stop and ask the user** — don't guess.
2. **Validate the SPEC entry in `docs/milestones/m{slug}/spec.md`.** It must contain all five required sections: **Intent**, **Inputs**, **Outputs**, **Edge cases**, **Acceptance criteria**. Use grep to confirm each subheading is present and non-empty (more than the heading itself). If any section is missing or empty, **stop** and surface a concrete request to the user: "spec entry at `docs/milestones/m{slug}/spec.md` is missing the following sections: …. Please complete the shard before The Loop runs." Do not invent the missing content.
...
- Dispatch the `planner` subagent for **$ARGUMENTS only** (per ADR-022). Scope: **write `docs/milestones/m{slug}/plan.md` only — DO NOT touch `docs/milestones/m{slug}/tests.md` in this dispatch** (per ADR-025). Pass `mode: PLAN` AND `slug: m{X}` at the top of the dispatch prompt.
- Pass: feature slug, expected `plan.md` shape (file structure, data models, components, design tokens, decisions to honor), the migration table for any obsolete IDs.
- When PLANNER returns: commit + push the `docs/milestones/m{slug}/plan.md` diff using prefix `docs(plan-<feature>): …` (per ADR-027). Do NOT pause for user review.
- **Auto-chain to Step 2.**

### Step 2 — TESTS (PLANNER, `mode: TESTS`)

- Dispatch the `planner` subagent **a second time** for **$ARGUMENTS only**. Scope: **write `docs/milestones/m{slug}/tests.md` only — derived from the just-written `m{slug}/plan.md`** (per ADR-025). Pass `mode: TESTS` AND `slug: m{X}` at the top of the dispatch prompt.
- Pass: feature slug, the just-written `m{slug}/plan.md` entry, IDs to start from, expected ID prefixes, expected GIVEN/WHEN/THEN coverage (success cases, failure cases, edge cases).
- When PLANNER returns: commit + push the `docs/milestones/m{slug}/tests.md` diff using prefix `docs(tests-<feature>): …` (per ADR-027).
- **Auto-chain to Step 3 (VERIFY).** No user gate (per ADR-041).

### Step 3 — VERIFY (VERIFIER, auto)

- Dispatch the `verifier` subagent for **$ARGUMENTS only** (per ADR-041). Pass: feature slug, the just-written `m{slug}/plan.md`, the just-written `m{slug}/tests.md`.
...
- Dispatch the `builder` subagent for **$ARGUMENTS only** (per ADR-013). Pass: feature slug, the milestone's `tests.md` shard path, both decisions paths (`m{slug}/decisions.md` + `_general/decisions.md`), the migration table for any obsolete IDs, commitlint constraints (lowercase subject, allowed types), the ADR-027 prefix convention (`test(<feature>):` for red, `feat(<feature>):` / `fix(<feature>):` for green).
...
- Dispatch the `evaluator` subagent. Pass: feature slug, base commit (last commit before BUILDER started), HEAD, the four BUILDER-flagged items, the `m{slug}/spec.md` + `m{slug}/tests.md` paths, both decisions paths, the VERIFIER PASS report (so EVALUATOR knows the design surface was already audited and can focus on code).
...
- **Never bundle phases.** PLAN and TESTS are two separate PLANNER dispatches per ADR-025. Never let one PLANNER dispatch touch both `plan.md` and `tests.md` (in the same milestone shard).
- **Never bundle features.** ADR-022 + ADR-013 are non-negotiable. One milestone slug per dispatch — the shard boundary IS the feature boundary. If `$ARGUMENTS` looks like multiple slugs, stop and ask.
- **Never touch the legacy monoliths.** `docs/spec.md`, `docs/plan.md`, `docs/tests.md`, `docs/decisions.md` are frozen reference. All reads + writes go to `docs/milestones/m{slug}/`. ADRs that emerge during EVAL go to either `m{slug}/decisions.md` (milestone-specific) or `_general/decisions.md` (cross-cutting).
...
- Spec edits — those are user-direct edits to `docs/milestones/m{slug}/spec.md` (creating the folder if new) and `phase1plan.md`, not pipeline runs (Phase 1 is user-owned per ADR-025).
- ADR additions that aren't tied to a feature ship (e.g. harness changes) — those are direct `docs(harness): …` commits by Main Claude, written to `_general/decisions.md` or the relevant milestone's `decisions.md`.
